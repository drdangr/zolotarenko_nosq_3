...
Поза вимогами ДЗ: невеликий веб-застосунок, що рекомендує фільми через **графові
ембеддинги** (FastRP) і **векторний пошук** (cosine). Замикає дугу курсу: документи
(Mongo) → вектори (Pinecone, HW2) → граф (Neo4j) → **граф ЯК вектори**.

- `gds.fastRP` будує вектор кожного фільму з його положення в графі `CO_RATED`;
- mean-centering прибирає спільну компоненту «популярність», лишаючи смак;
